/* -----------------------------------------------------------------------------
 *
 * Module      :  Dirty CPP hackery for OpenGL's basic types
 * Copyright   :  (c) Sven Panne 2009 + the FFI task force
 ...
 *
 * This header should only define preprocessor macros! It is basically a
 * slightly modified version of CTypes.h and Typeable.h from GHC's base package.
 *
 * -------------------------------------------------------------------------- */
...
instance Read T where { \
   readsPrec p s = map (\(x, t) -> (T x, t)) (readsPrec p s) }
...
instance Enum T where { \
   ...
   enumFrom       (T i)             = map T (enumFrom i) ; \
   enumFromThen   (T i) (T j)       = map T (enumFromThen i j) ; \
   enumFromTo     (T i) (T j)       = map T (enumFromTo i j) ; \
   enumFromThenTo (T i) (T j) (T k) = map T (enumFromThenTo i j k) }
